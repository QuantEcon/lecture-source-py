...
The **orthogonal complement** of linear subspace :math:`S \subset RR^n` is the set :math:`S^{\perp} := \{x \in \mathbb R^n \,:\, x \perp S\}`
...
Intuitively, we may not be able find a :math:`b` that satisfies all :math:`n` equations
