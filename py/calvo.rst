...
-  two Bellman equations, one that expresses the private sector's  expectation of future inflation
   ...
   describes  the value function of a planner
...
- :math:`\theta_t = p_{t+1} - p_t` be the rate of inflation between :math:`t` and :math:`t+1`

- :math:`\mu_t = m_{t+1} - m_t` be the rate of growth of nominal balances
...
We can represent :math:`v_0` recursively via
...
At this point :math:`\vec \mu` is an arbitrary exogenous policy
...
In two of our models, there is a single policy maker that chooses a sequence
:math:`\{\mu_t\}_{t=0}^\infty` once and for all, taking into account how
:math:`\mu_t`'s at each date affect earlier components of household
utilities
...
synthetic **promised inflation rates** that are just tools for
...
at time :math:`0`, a government chooses a **constant** sequence
:math:`\mu_t = \check \mu` for all :math:`t \geq 0` to maximize

.. math::  U(-\alpha \check \mu) - \frac{c}{2} \check \mu^2

Here we have imposed the perfect foresight outcome that
...
government in order to highlight the role of time-variation of
:math:`\mu_t` in generating time inconsistency of a Ramsey plan


Markov Perfect Governments
===========================

We now  change the timing protocol by considering a government that
...
Further, the government at :math:`t` believes that :math:`\bar \mu` is
...
In settings in which governments choose sequentially, many economists
...
Research by Abreu :cite:`Abreu`,  Chari and Kehoe :cite:`chari1990sustainable`  :cite:`stokey1989reputation`, and :cite:`Stokey1991` discovered conditions under which a Ramsey plan can be rescued from the complaint that it is not credible
...
-  The government chooses :math:`\{\mu_t\}_{t=0}^\infty` not once and
   for all at :math:`t=0` but sequentially
...
-  If the government **disappoints** private agents by setting
...
The idea is to use the self-enforcing plan as a continuation plan when
...
insisting that the government set :math:`\mu_t` to a value yielding low
one-period utilities to the household for a long time, after which the
government forever does things yielding high one-period utilities

-  low one-period utilities early are a **stick**

-  high one-period utilities later are a **carrot**

Consider a plan :math:`\vec \mu^A` in which the government sets
...
The following example implements Abreu's idea
...
To confirm that the plan :math:`\vec \mu^A` is **self-enforcing**, the right panel
plots an object that we call :math:`V_t^{A,D}`, defined in the second line of equation :eq:`eq_old10` above
