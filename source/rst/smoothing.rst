...
This is equivalent to saying that  it costs :math:`\beta^{-1}` of  time :math:`t` consumption to buy one unit of consumption at time :math:`t+1` for sure.


So :math:`\beta^{-1}` is the price of a one-period risk-free claim to consumption next period.
...
*  In an :math:`N` state Markov state version,  :math:`N` such securities are traded each period.
...
which  implies :math:`c_t = \bar c` for some :math:`\bar c`.
...
where :math:`\bar b_0` is an initial level of the consumer's debt due at time :math:`t=0`, specified
...
To determine :math:`\bar c`, we shall deduce implications of the consumer's budget constraints in each Markov state today and  our guess :eq:`eq_guess` about the consumer's debt level choices.
