...
one-period payoff by setting :math:`\mu =0`.
...
its setting of :math:`\mu_t` would bring for continuation  government payoffs via  alterations  in the private sector's expectations.
...
assumption that governments at times :math:`t \geq 0` always act to
...
Thus, where :math:`\vec v^A` is the value associated with a self-enforcing plan :math:`\vec \mu^A`, 
...
plot an object that we call :math:`V_t^{A,D}`, defined in the key inequality in the second line of equation :eq:`eq_old10` above.
